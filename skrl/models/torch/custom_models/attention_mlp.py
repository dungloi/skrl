...
该模块用于 ``ego`` 与 ``other_*`` 多体交互建模，包含：

- ``AttentionMLPPolicy``：通过 Cross Attention 提取交互特征并输出动作均值
- ``AttentionMLPValue``：复用交互建模流程并输出标量 value

网络结构由 ``network.attention``、``network.embedding``、``network.mlp``
三段配置驱动。
"""
...
class AttentionMLPPolicy(GaussianMixin, Model):
    """使用 ego-other attention 与 MLP 动作头的 Gaussian policy。

    期望的网络配置:
        network.attention + network.embedding + network.mlp
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        network_cfg = _normalize_network_root(
            ...
            required_keys=_ATTENTION_MLP_REQUIRED_KEYS,
            model_name=self.__class__.__name__,
        )
        attention_cfg = _normalize_attention_cfg(network_cfg.get("attention"))
        embedding_cfg = _normalize_embedding_cfg(network_cfg.get("embedding"))
        ...
        # 4. 定义 "ego", "other" 的特征映射层
        self.ego_embedding = _build_embedding(
            input_dim=ego_input_dim,
            output_dim=embed_dim,
            use_layernorm=embedding_cfg["use_layernorm"],
            activation_factory=embedding_cfg["activation_factory"],
        )
        self.other_embedding = _build_embedding(
        ...
        _print_inputs(
            ...
            action_space,
            device,
            space_priority=_ATTENTION_MLP_SPACE_PRIORITY,
        )
        _print_gaussian_settings(
        ...
        _print_subconfig(
            "embedding",
            [
                ("ego_in_dim", ego_input_dim),
                ("other_in_dim", other_input_dim),
                ("use_layernorm", embedding_cfg["use_layernorm"]),
                ("activation", embedding_cfg["activation"]),
            ],
        )
        _print_subconfig(
            "mlp",
...
class AttentionMLPValue(DeterministicMixin, Model):
    """使用 ego-other attention 与 MLP 头的 deterministic value 模型。
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 网络结构定义
        # ----------------------------------------
        # 1. 解析输入空间维度（state_space 是一个 Dict，包含 "ego" 和 "other_i"）
        # 解析 "other_i" 的数量和 shape，假设它们都是同样的 shape
...
__all__ = ["AttentionMLPPolicy", "AttentionMLPValue"]
