...
@pytest.mark.parametrize("agent_class", [PPO, PPO_RNN])
@pytest.mark.parametrize(
    ("corrupt_from", "expected_effective_minibatches", "expected_observed_minibatches", "expected_scheduler_calls"),
    [(0, 0, 1, 0), (2, 1, 2, 1)],
)
def test_kl_early_stop_ends_all_epochs_and_does_not_reschedule_without_an_optimizer_step(
    ...
    _collect_rollout(agent)
    with torch.no_grad():
        agent.memory.get_tensor_by_name("log_prob").flatten(0, 1)[corrupt_from:].sub_(5.0)
